<template>
    <div id='lists'>
         <div class='itemList' v-swipe  v-for='(item,index) in itemList' :data-index='index' @click='routerTo(item,$event)' >
           <div class='middle' >
             <h3>{{item.service.serviceName}}</h3>
             <p>{{item.firstUserName}}：{{item.firstContent}}</p>
             </div>
            <div :data-content='numFilter(item.remindNum)' :class='item.remindNum>0?"badge":""' class='left '>
              <img :src='item.service.url'></div>
            <div class='right' >
...
// import Vue from 'vue';
import { timeFilter ,urlImg} from '@/common/js/mixin'
...
.itemList {
  ...
  .middle {
    text-align: left;
    padding: 0.12rem 2.12rem 0.12rem 1.02rem;
